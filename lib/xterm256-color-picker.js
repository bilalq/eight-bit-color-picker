...
(function (root, factory) {
  ...
  XTerm256ColorPicker.prototype.buildColorMapUI = function() {
    ...
    var selectedColorEl = this.el.querySelector('.xt256cp-selection')
      ...
      , previewColorEl = this.el.querySelector('.xt256cp-color-preview')
     ...
     */
    var updateColor = (function(color, previewOnly) {
      ...
      eightBitTextEl.value = eightBitColor
      hexTextEl.value = twentyFourBitColor
      previewColorEl.style.background = twentyFourBitColor
    }).bind(this)
    ...
    var restoreColor = (function() {
      previewColorEl.style.background = hexTextEl.value = this.getHexColor()
      eightBitTextEl.value = this.get8BitColor()
    }).bind(this)

    // Hover handling for color preview
    colorMapEl.addEventListener('mouseover', function(e) {
      var eightBitColor = e.target.dataset.eightBitColor
      updateColor(eightBitColor, true)
    })

    // Restore preview to selected color when cursor leaves the color map
    colorMapEl.addEventListener('mouseleave', restoreColor)

    // Click handling for color selection
    colorMapEl.addEventListener('click', function(e) {
      updateColor(e.target.dataset.eightBitColor)
    })

    // Update color when text input is edited
    eightBitTextEl.addEventListener('keyup', function(e) {
      updateColor(this.value)
    })

    // Restore & normalize color when leaving focus of text input
    eightBitTextEl.addEventListener('blur', restoreColor)
  }
